...
# Log file for packages not found
LOGFILE="package_errors.log"
: > "$LOGFILE"  # Clear previous log

# Function to install a single package using pacman, yay, or paru
install_package() {
  local pkg="$1"
  ...
  if pacman -Si "$pkg" &>/dev/null; then
    ...
    echo "Error: Package '$pkg' not found in pacman, yay, or paru." | tee -a "$LOGFILE"
  fi
}

# Function to process a list of package names
install_packages() {
  for pkg in "$@"; do
    install_package "$pkg"
  done
...
# Define your list of unique packages here
packages=(
  aic94xx-firmware
  ast-firmware
  gnome-firmware
  ...
  python-wordcloud
  python-flair
  python-bert-extractive-summarizer
  freemind
  freeplane
  vyms
  docear
...
# Install packages using the defined function
